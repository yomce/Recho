...
const VideoEditScreen: React.FC<{ route: VideoEditScreenRouteProp }> = ({
  ...
  const createCollage = async () => {
    ...
    try {
      ...
      const encoder =
        Platform.OS === 'ios'
          ? // Intel Mac 환경에서는 아래 h264_videotoolbox 대신 소프트웨어 인코더(예: libx264) 사용을 고려하세요.
            '-c:v h264_videotoolbox -b:v 2M'
          : // '-c:v mpeg4' // 주석 처리: Intel Mac 호환성 옵션
            '-c:v h264_mediacodec';

      // [수정] 안정성을 위해 preset을 ultrafast로 변경하고 crf 옵션 제거
      const command = `${inputCommands} -filter_complex "${filterComplexString}" -map "[v]" ${audioMapCommand} ${encoder} -preset ultrafast -shortest "${outputPath}"`;
