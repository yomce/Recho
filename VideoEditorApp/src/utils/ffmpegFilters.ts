...
export const generateCollageFilterComplex = (editData: EditData): string[] => {
  ...
  if (audioOutputs.length > 0) {
    filterComplex.push(
      `${audioOutputs.join('')}amix=inputs=${videoCount}:duration=shortest[a]`,
    );
  }
  ...
  console.log(
    `[FFmpegFilters] Calculated shortest duration for collage: ${shortestDuration} seconds`,
  );
  ...
  const FRAME_WIDTH = ensureEven(
    (COLLAGE_WIDTH - (NUM_COLS + 1) * PADDING) / NUM_COLS,
  );
  ...
  filterComplex.push(
    `color=c=black:s=${bg_width}x${bg_height}:d=${shortestDuration}[bg]`,
  );
  ...
  trimmers.forEach((trimmer, i) => {
    ...
    if (cornerRadius > 0) {
      filterComplex.push(
        `color=c=black:s=${FRAME_WIDTH}x${frame_h}[mask${i}_base]`,
      );
      filterComplex.push(
