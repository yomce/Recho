...
export const SessionDetail: React.FC<SessionDetailProps> = ({ item, postId, applicationEnsembleList }) => {
  ...
  const applierUsernames = useMemo(() => {
    return applicationEnsembleList
      ...
      .map((app) => app.username); // user 객체 안의 username을 추출
  ...
  const handleApply = async () => {
    ...
    try {
      const response = await axiosInstance.post(`application/${postId}/${item.sessionId}}`);
      alert('모집 공고에 지원했습니다!');
  ...
  const handleCancel = async () => {
    ...
    try {
      const response = await axiosInstance.delete(`application/${postId}/${item.sessionId}`);
